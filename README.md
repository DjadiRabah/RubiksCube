# Rubik's Cube 
## Description
Projet d'une application Android de Rubik's Cube permettant de jouer ou de lancer la résolution d'un Rubik's Cube via saisie manuelle ou photographie.

## Versions
Version 1.0 17/10/2018
 - Ajouts de fonctionnalités permettant :
  - d'afficher un rubik's Cube sous forme de patron
  - de mélanger le cube avec le boutton "Mélanger"
  - de faire tourner les différentes faces du Rubik's Cube
  - d'annuler la dernière rotation avec le boutton "Retour"
 ![solvedCube](https://dwarves.iut-fbleau.fr/git/djadi/RubiksCube/raw/master/Ressources/SolvedCube.png)
 ![unsolvedCube](https://dwarves.iut-fbleau.fr/git/djadi/RubiksCube/raw/master/Ressources/UnsolvedCube.png)
 
 Version 2.0 03/03/2019  
 - Ajouts de fonctionnalités permettant :   
     -de résoudre un cube 3x3

## Langages
[<img src="https://camo.githubusercontent." width="10%" height="10%">](https://www.java.com/)

## Logiciels

[![eclipse](https://dwarves.iut-fbleau.fr/git/djadi/RubiksCube/raw/master/Ressources/eclipse_builton.png)](https://www.eclipse.org/)


## Tuteur
**Monnerat Denis** _alias_ [@monnerat](https://dwarves.iut-fbleau.fr/git/monnerat)

## Auteurs
**ABREU DIAS Thomas** _alias_ [@abreudia](https://dwarves.iut-fbleau.fr/git/abreudia)  
**BLANCHON Maxime** _alias_ [@blanchon](https://dwarves.iut-fbleau.fr/git/blanchon)  
**DJADI Rabah** _alias_ [@djadi](https://dwarves.iut-fbleau.fr/git/djadi)  
**GARRIGUES Thomas** _alias_ [@garriguet](https://dwarves.iut-fbleau.fr/git/garriguet)  
**LE ROUX DUPEYRON Amaury** _alias_ [@lerouxdu](https://dwarves.iut-fbleau.fr/git/lerouxdu)  
Étudiants en seconde année de DUT informatique à l'IUT de Fontainebleau.

 