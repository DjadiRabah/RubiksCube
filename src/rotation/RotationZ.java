	...
	public void rotate(Cube cube, int direction, int index, int n)
	{
		if ((n%4) != 0)
		{
			...
			else if (((n + 1) % 4) == 0)
			{
				switch (direction) 
				{
		            case CLOCKWISE : this.rotate(cube, Rotation.COUNTERCLOCKWISE, index, 1); break;
		            case COUNTERCLOCKWISE: this.rotate(cube, Rotation.CLOCKWISE, index, 1);  break;
		            default: break;
				}
			}
			else
			{
				if (index == 0)
				{
					new RotationSquare().rotate(cube.getSquare(Cube.FRONT), Rotation.CLOCKWISE, 2);
				}
				else if(index == cube.getSize() - 1)
				{
					new RotationSquare().rotate(cube.getSquare(Cube.BACK), Rotation.CLOCKWISE, 2);
				}
				Piece[] temp = cube.getSquare(Cube.TOP).getLayer(cube.getSize() - index - 1);
            	cube.getSquare(Cube.TOP).setLayerReverse(cube.getSize() - index - 1, cube.getSquare(Cube.DOWN).getCol(index));
            	cube.getSquare(Cube.DOWN).setLayerReverse(index, temp);
            	temp = cube.getSquare(Cube.LEFT).getCol(cube.getSize() - index - 1);
            	cube.getSquare(Cube.LEFT).setColReverse(cube.getSize() - index - 1, cube.getSquare(Cube.RIGHT).getLayer(index));
            	cube.getSquare(Cube.RIGHT).setColReverse(index, temp);
			}
		}
	}
}
