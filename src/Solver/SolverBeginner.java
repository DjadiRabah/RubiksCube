...
public class SolverBeginner implements Solver
{
	...
	private void setWhiteCrossTopFront(Cube cube)
	{
		this.setEdges(cube);
		...
		if((this.getEdgeColor(Cube.TOP, 2, 1) != colorTop) || ((this.getEdgeColor(Cube.FRONT, 0, 1) != colorFront)))
		{
			...
			if((edgePosition[0] == Cube.LEFT) && ((edgePosition[1] == Cube.FRONT)))
				cube.rotate(Rotation.CLOCKWISE, 0);
			else if((edgePosition[0] == Cube.RIGHT) && ((edgePosition[1] == Cube.FRONT)))
				cube.rotate(Rotation.COUNTERCLOCKWISE, 0);
			else if((edgePosition[0] == Cube.DOWN) && ((edgePosition[1] == Cube.FRONT)))
				cube.rotate(Rotation.CLOCKWISE, 0);
			else if((edgePosition[0] == Cube.LEFT) && ((edgePosition[1] == Cube.TOP)))
				cube.rotate(Rotation.DOWN, 0);
			else if((edgePosition[0] == Cube.RIGHT) && ((edgePosition[1] == Cube.TOP)))
				cube.rotate(Rotation.DOWN, 2);
			else if((edgePosition[0] == Cube.FRONT) && ((edgePosition[1] == Cube.TOP)))
				cube.rotate(Rotation.CLOCKWISE, 0);
			this.setWhiteCrossTopFront(cube);
		}
	}
